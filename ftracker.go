...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	speed := meanSpeed(action, duration)
	// Потраченные калории
	calories := ((walkingCaloriesWeightMultiplier*weight + (math.Pow(float64(speed), 2.0)/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return calories
}
...
func swimmingMeanSpeed(lengthPool, countPool int, duration float64) float64 {
	...
	return float64(lengthPool)*float64(countPool)/mInKm/duration + swimmingCaloriesMeanSpeedShift
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	//Пораченные калории
	calories := speed*swimmingCaloriesWeightMultiplier*weight + duration
	return calories
